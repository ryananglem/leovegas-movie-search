...
export const requestSetFavourite = (id:string, favourite: boolean): ActionCreator => ({
...
export const setFavourite: any = (id: string, favourite: boolean) => async (dispatch: any, getState: any): Promise<void> => {
    try {
        ...
        // @ts-ignore
        const setFavouriteResponse = await fetch(apiUrl(`account/${session.account.id}/favorite`, `session_id=${session.id}`), {
            method: 'POST',
            mode: 'cors', 
...
export const getFavouritesList = () => async (dispatch: any, getState: any): Promise<void> => {
    try {
        dispatch(requestGetFavouritesList())
        const session = getState().authorisation

        // @ts-ignore
        const favouritesResponse = await fetch(apiUrl(`account/${session.account.id}/favorite/movies`, `session_id=${session.id}`))
